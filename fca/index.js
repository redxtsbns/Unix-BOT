...
var logger = require('../unix-utils/index.js')
...
function buildAPI(globalOptions, html, jar) {
  ...
  if (html.indexOf("/checkpoint/block/?next") > -1) {
    logger("⌞ 𝙻𝙾𝙶𝙸𝙽 ⌝ » Checkpoint detected. Please log in with a browser to verify.");
  }

  var userID = maybeCookie[0].cookieString().split("=")[1].toString();
  logger(`⌞ 𝙻𝙾𝙶𝙸𝙽 ⌝ » 𝙱𝙾𝚃 𝙸𝙳:  ${userID}`);
  ...
  if (oldFBMQTTMatch) {
    ...
    logger(`⌞ 𝙻𝙾𝙶𝙸𝙽 ⌝ » 𝚁𝙴𝙲𝙴𝙸𝚅𝙴𝙳 𝚁𝙴𝙶𝙸𝙾𝙽: ${region}`);
  } else {
                      ...
                      utils
                        ...
                        .then(res => {
                          try {
                            ...
                            clearInterval(checkVerified);
                            logger("𝙻𝙾𝙶𝙸𝙽 » ", "Verified from browser. Logging in...");
                            if (callback === prCallback) {
                              callback = function (err, api) {
                                if (err) {
                                  return prReject(err);
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
  ...
  mainPromise
    .then(function () {
      logger("⌞ 𝙻𝙾𝙶𝙸𝙽 ⌝ » 𝙻𝙾𝙶𝙸𝙽 𝙳𝙾𝙽𝙴.")
      return callback(null, api);
    })
    .catch(function (e) {
      logger("⌞ 𝙻𝙾𝙶𝙸𝙽 ⌝ »", e.error || e);
      callback(e);
    });
}
